...
def connectivity_plot(matrix):
    """Draw image for visualizing a weight matrix.
        (from original matplotlib docs)"""
    fig = plt.figure('conn')
    ax = plt.gca()
    ...
    major_ticks_x = np.arange(-.5, matrix.shape[1], 1)
    major_ticks_y = np.arange(-.5, matrix.shape[0], 1)
    plt.xticks([])
    plt.yticks([])
    ax.set_xticks(major_ticks_x)
    ax.set_yticks(major_ticks_y)
...
def filtered_average_connectivity(path, cutoff_hemisphere_connections=2,
    ...
    neuron_names = np.genfromtxt(f'{path}/connectivityMatrix.csv',
                                dtype=np.unicode_,
                                delimiter=',',
                                skip_header=1,
                                usecols=(0),
    ...
    left_neurons_indices = \
                            [("LEFT" in name) for name in neuron_names]
    right_neurons_indices = \
                            [("RIGHT" in name) for name in neuron_names]
    left_neurons_names = [name.replace(" LEFT", "") \
                            for name in neuron_names \
                            if ("LEFT" in name)]
    right_neurons_names = [name.replace(" RIGHT", "") \
                              for name in neuron_names \
                              if ("RIGHT" in name)]
    assert left_neurons_names == right_neurons_names
    #Get connectivity values (between and within hemispheres)
    connectivity_left_left = connectivity_total[
                                            left_neurons_indices, :] \
                                            [:, left_neurons_indices]
    connectivity_right_right = connectivity_total\
                                            [right_neurons_indices, :] \
                                            [:, right_neurons_indices]
    connectivity_left_right = connectivity_total\
                                            [left_neurons_indices, :] \
                                            [:, right_neurons_indices]
    connectivity_right_left = connectivity_total\
                                            [right_neurons_indices, :] \
                                            [:,  left_neurons_indices]
...
def both_hemispheres_connectivity(path, cutoff=2):
    ...
    right_neurons_indices = \
                            [("RIGHT" in name) for name in neuron_names]
    single_neurons_indices = \
                            [("SINGLE" in name) for name in neuron_names]
    left_neurons_names = [name.replace(" LEFT", "") \
                            for name in neuron_names \
                            if ("LEFT" in name)]
    right_neurons_names = [name.replace(" RIGHT", "") \
                              for name in neuron_names \
                              if ("RIGHT" in name)]
    single_neurons_names = [name.replace(" SINGLE", "") \
                              for name in neuron_names \
                              if ("SINGLE" in name)]
    assert left_neurons_names == right_neurons_names
    #Get connectivity values (between and within hemispheres)
    ...
    # disregard autapses:
    print('Autapses are ignored. Max autapses value was', 
            np.amax(np.diag(connectivity)), '.')
    np.fill_diagonal(connectivity, 0)

    return neuron_names, connectivity

def first_order_trial(valence,
                      doUS,
                      doC,
                      input_size=70,
                      input_fraction=.1,
    ...
    cs_start = int(np.random.randint(cs_start_window[0],
                                     cs_start_window[1]) / dt)
    cs_test_start = int(np.random.randint(cs_test_start_window[0],
                                    cs_test_start_window[1]) / dt)
    activated_subset = select_random(input_size, int(input_size * input_fraction))
    activated_subset_for_control = select_random(input_size,
                                               int(input_size * input_fraction))
    ...
    if doC:
        activated_neurons[cs_test_start:(cs_test_start + cs_duration),:] = \
    ...
    if returntimes:
        return activated_neurons, unconditioned_stimulus, output,\
               cs_start, us_start, cs_test_start
    return activated_neurons, unconditioned_stimulus, output

def first_order_trials(valence_values=[0, 1],
                      doUS=True,
                      doC=False,
                      num_trials=2,
                      input_size=70,
    ...
    for i in range(num_trials):
        ...
        if doUS:
            if valence > 0:
                unconditioned_stimulus[i, 
                                   us_start:(us_start + us_duration), 
                                   0] = 1
            else:
                unconditioned_stimulus[i, 
                                   us_start:(us_start + us_duration), 
...
def first_order_trials_original_shape(valence_values=[-1, 1],
                      doUS=True,
                      doC=False,
                      num_trials=10,
                      input_size=70,
    ...
    for i in range(num_trials):
        valence = np.random.choice(valence_values)
        activated_subset = select_random(input_size, int(
                                                input_size * input_fraction))
        activated_subset_for_control = select_random(input_size,
                                            int(input_size * input_fraction))
        print(valence)
        if doC:
            activated_neurons[cs_test_start:(cs_test_start + cs_duration),
                              :,
                              i] = activated_subset_for_control
        else:
            activated_neurons[cs_test_start:(cs_test_start + cs_duration),
                              :,
                              i] = activated_subset
        if doUS:
            if valence > 0:
                unconditioned_stimulus[us_start:(us_start + us_duration), 
                                        0,
                                        i] = 1
            else:
                unconditioned_stimulus[us_start:(us_start + us_duration), 
    ...
    if returntimes:
        return activated_neurons, unconditioned_stimulus, target_output,\
               cs_start, us_start, cs_test_start
    return torch.from_numpy(activated_neurons),\
            torch.from_numpy(unconditioned_stimulus),\
            torch.from_numpy(target_output)

def odor_valence_pairs(num_cells,
                       num_odors=1,
                       coding_level=.1,
                       valence_values=[-1, 1]):
...
def first_order_xp_2(
    ...
    for i in range(num_trials):
        ...
        activated_KCs[cs_start:(cs_start + cs_duration), :, i] = \
                                                                activated_subset
        if doC:
            activated_KCs[cs_test_start:(cs_test_start + cs_duration),
                              :,
                              i] = activated_subset_for_control
        else:
            activated_KCs[cs_test_start:(cs_test_start + cs_duration),
        ...
        if doUS:
            if valence > 0:
                ...
                unconditioned_stimulus[us_start:(us_start + us_duration), 
                                        ...
                                        i] = 1
        if doUS and not doC:
            target_output[cs_test_start:(cs_test_start + cs_duration), i] = \
                                                                        valence

    return torch.from_numpy(activated_KCs),\
            torch.from_numpy(unconditioned_stimulus),\
            torch.from_numpy(target_output)
...
# def odors_set_generation(odors, mabons=70, activated_mbons=.5):
#     activation_matrix = np.zeros((odors,activated_mbons, 2)) # left/right
#     for i in range odors:
#         activation_matrix[i, :] = 
...
if __name__=="__main__":
    ...
    valences = np.zeros((7))
    for i in range(7):
        odors[:, i] = select_random(70, int(70 * .1))
